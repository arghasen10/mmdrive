...
merged_df = pd.read_pickle('./Aqu_dop.pkl')
...
class rf_model:
    ...
    def PoolOp(self, img, ksize=(4, 16), op=np.mean):
    ...
    def subapply(self, v, op):
        return self.PoolOp(v, ksize=(4, 16), op=op)
...
frame_stack = 10
df = merged_df[['rangedoppler', 'activity']]
data = np.array(df['rangedoppler'].values.tolist())
label = df['activity'].values
label = convert_to_neumeric(label)
data, label = StackFrames(data, label, frame_stack)
mask = label != 9
X_train, X_test, y_train, y_test = split_dataset(data[mask].transpose(0, 2, 3, 1), label[mask])
...
labels = ['Drinking', 'Fetching forward', 'Anomaly in Steering', 'Nodding', 'Picking drops', 'Using Phone',
          'Turning back', 'Talking left', 'Yawning']
